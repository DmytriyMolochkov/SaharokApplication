...
using Saharok.Properties;

namespace Saharok.Model
{
    public class MyFileAssociation 
    {
        public MyFileAssociation()
        {
            if (!File.Exists(System.IO.Path.Combine(Environment.CurrentDirectory, "Group icon", "fileicon.ico")))
            {
                Directory.CreateDirectory(System.IO.Path.Combine(Environment.CurrentDirectory, "Group icon"));
                using (FileStream stream = new FileStream(System.IO.Path.Combine(Environment.CurrentDirectory, "Group icon", "fileicon.ico"), FileMode.Create))
                {
                    Resources.fileicon.Save(stream);
                }
            }
        ...
        private void CheckFileAssociation()
        {
            ...
            if (!assoc.Exists
                || (assoc.ID == null || assoc.Description == null || assoc.DefaultIcon == null || assoc.Executable == null)
                || (assoc.ID != "Saharok" || assoc.Description != "Saharok file association"
                || assoc.DefaultIcon.IconPath != System.IO.Path.Combine(Environment.CurrentDirectory, "Group icon", "fileicon.ico")
                || assoc.Executable.Path != @"""" + System.IO.Path.Combine(Environment.GetCommandLineArgs()[0]) + @""""))
            {
                CreateAssociation(assoc);
            }
        }
        private void CreateAssociation(AF_FileAssociator assoc)
        {
            try
            {
                assoc.Create("Saharok",
                    "Saharok file association",
                    new ProgramIcon(System.IO.Path.Combine(Environment.CurrentDirectory, "Group icon", "fileicon.ico")),
                    new ExecApplication(System.IO.Path.Combine(Environment.GetCommandLineArgs()[0])),
                    new OpenWithList(new string[] { "Saharok" }));
        ...
        string GetProgID
        {
            get
            {
                ...
                if (Registry.ClassesRoot.OpenSubKey(Extension,
                    RegistryKeyPermissionCheck.ReadWriteSubTree,
                    RegistryRights.ReadKey) != null)
                {
                    if (Registry.ClassesRoot.OpenSubKey(Extension,
                        RegistryKeyPermissionCheck.ReadWriteSubTree,
                        RegistryRights.ReadKey).GetValue("") != null)
                    {
                        toReturn = Registry.ClassesRoot.OpenSubKey(Extension,
                            RegistryKeyPermissionCheck.ReadWriteSubTree,
                            RegistryRights.ReadKey).GetValue("").ToString();
                    }
                }
            ...
            if (progID != null)
            {
                if (defaultIcon.IsValid && execApp.IsValid)
                {
                    ...
                    if (execApp != null && execApp.IsValid)
                        key.CreateSubKey(@"Shell\Open\Command").SetValue("", @"""" + execApp.Path + @""" "" %1""", RegistryValueKind.ExpandString);
                    else
                        ...
                        RegistryRights.ReadKey) != null)
                    {
                            ...
                            RegistryRights.ReadKey) != null)
                        {
                                ...
                                RegistryRights.ReadKey).GetValue("") != null)
                            {
                                ...
                                execApp = new ExecApplication(path.Substring(0, path.LastIndexOf('%') - 3));
                            }
                        }
                        else
                        {
                        ...
                        RegistryRights.ReadKey) != null)
                    {
                            ...
                            RegistryRights.ReadKey) != null)
                        {
                            ...
                            foreach (string file in Registry.ClassesRoot.OpenSubKey(GetProgID + @"\OpenWithList",
                                 RegistryKeyPermissionCheck.ReadWriteSubTree,
                                 RegistryRights.ReadKey).GetSubKeyNames())
                            {
                                list.Add(file);
                            }
